...
const app = express();
app.get('/', (req, res) => res.send('Sistema Operacional'));
...
async function iniciarBot() {
    console.log("--- TENTANDO INICIAR BOT ---"); // Log de diagnóstico
    
    try {
        const { state, saveCreds } = await useMultiFileAuthState('auth_info');
        const sock = makeWASocket({
            auth: state,
            logger: pino({ level: 'debug' }), // Alterado para debug para ver erros
        ...
        sock.ev.on('connection.update', (update) => {
            ...
            if (update.connection === 'close') console.log("❌ CONEXÃO FECHADA. Tentando reconectar...");
        });
        ...
        console.log("ERRO CRÍTICO AO INICIAR: ", err);
    }
}
